<?php 
    ...
    if ($nomC=="tous"){
      $requete="SELECT * FROM rdv ORDER BY id LIMIT $premier,$size";
      $sqlCount="SELECT COUNT(*) AS nbrU FROM rdv";
    }else {
      $requete="SELECT * FROM rdv WHERE nom like '%$nomC%' or prenom like '%$nomC%'  or telephone like '%$nomC%'or jour like '%$nomC%'
     or mois like '%$nomC%' or annee like '%$nomC%'  LIMIT $premier,$size ";
  
      $sqlCount="SELECT COUNT(*) AS nbrU FROM rdv WHERE nom like '%$nomC%' or prenom like '%$nomC%'  or telephone like '%$nomC%'or jour like '%$nomC%'
      or mois like '%$nomC%' or annee like '%$nomC%'";
    }
...
<div id="mySidenav" class="sidenav">
     ...
     <a href="#" id="Lrdv">Liste RDV</a>
       <a href="ajouter_rdv.php"id="Ardv">Ajouter RDV</a>
       <a href="nouveaupatient.php"id="np">Nouveau Patient</a>
   </div>
<div class="container "style="margin-top:1%">
  <form class="form-inline" action="ListeRdvm.php" method="get" style="border: 2px width:444px;solid #4e97ff; ">
  ...
  <button type="submit" class="btn btn-default" style="margin-top: -57px; margin-left: 97%; width: 53px; height: 34px;">
    <span class="glyphicon glyphicon-search"></span> 
        ...
        <div class="panelpanelprimary"style="border: 2px solid #0000004a; width: 1210px; margin-left: 10%; margin-top: 0px;">
            <div class="panel-heading" style="color:white; background-color:#0096FF;text-align: center;"> <strong>Liste des rendez-vous(<?php echo $nbreUser ?> rendez-vous)</strong> </div>
            <div class="panel-body" style="padding:0px;">
                <table class="table table-striped">
                    <thead>
   <th>N°patient</th><th>Nom</th><th>Prenom</th><th>telephone</th><th>Date de RDV</th><th>Time</th><th>Action</th>
                   </thead>
                        <tbody>
                        <?php
                        ...
                        ?>
                             <tr>
                                ...
                                <td>
                                    <a href="modifier_patient.php?id=<?=$user['id']?>">
                                    <span class="glyphicon glyphicon-edit"></span>
                            </a>
          <a onclick="return confirm (' etes vous sur de vouloir suprimer le rendez-vous ? ')" href="../bdd/supprim_pat.php?id=<?=$user['id']?>">
                ...
                <nav>
    <ul class="pagination" style=" margin: 62px 1px 1px;  ">
    <li class="page-item <?= ($currentPage == 1) ? "disabled" : "" ?>">
            <a href="ListeRdvm?page=<?= $currentPage - 1 ?>" class="page-link">Précédente</a>
        </li>
        <?php for($i= 1; $i <= $pages; $i++): ?>
            <!-- Lien vers chacune des pages (activé si on se trouve sur la page correspondante) -->
            <li class="page-item <?php if($i == $currentPage)  echo "active";  ?>">
                <a href="ListeRdvm.php?page=<?= $i ?>" class="page-link"><?= $i ?></a>
            </li>
        <?php endfor ?>
           <!-- Lien vers la page suivante (désactivé si on se trouve sur la dernière page) -->
           <li class="page-item <?= ($currentPage == $pages) ? "disabled" : "" ?>">
            <a href="ListeRdvm?page=<?= $currentPage +1 ?>" class="page-link">Suivante</a>
        </li>
