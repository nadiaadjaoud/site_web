<?php 
    ...
    if ($nomC=="tous"){
      $requete="SELECT * FROM patient ORDER BY id LIMIT $premier,$size";
      $sqlCount="SELECT COUNT(*) AS nbrU FROM patient";
    }else {
      $requete="SELECT * FROM patient WHERE nom like '%$nomC%' or prenom like '%$nomC%'  or telephone like '%$nomC%'or adresse like '%$nomC%'
     or email like '%$nomC%' or jour like '%$nomC%'or mois like '%$nomC%'or annee like '%$nomC%'or sit_fam like '%$nomC%'or sexe like '%$nomC%' LIMIT $premier,$size ";
  
      $sqlCount="SELECT COUNT(*) AS nbrU FROM patient WHERE  nom like '%$nomC%' or prenom like '%$nomC%'  or telephone like '%$nomC%'or adresse like '%$nomC%'
      or email like '%$nomC%' or jour like '%$nomC%'or mois like '%$nomC%'or annee like '%$nomC%'or sit_fam like '%$nomC%'or sexe like '%$nomC%'";
    }
...
.icon{
    ...
    <header style=" margin-left: 1px;">
     ...
     <a href="#" id="LPAT">Liste patients</a>
...
<div class="container "style="margin-top:1%">
  <form class="form-inline" action="ListePat.php" method="get" style="border: 2px width:444px;solid #4e97ff; margin-left: 69PX; ">
  ...
  <button type="submit" class="btn btn-default" style="margin-top: -57px; margin-left: 97%; width: 53px; height: 34px;">
    <span class="glyphicon glyphicon-search"></span> 
        ...
        <div class="panelpanelprimary"style="border: 2px solid #0000004a; width: 1210px; margin-left: 6%; margin-top: 15px;">
            <div class="panel-heading" style="color:white; background-color:#0096FF;text-align: center;"> <strong>Liste des patients (<?php echo $nbreUser ?> Patient)</strong> </div>
            <div class="panel-body" style="padding:0px;">
                <table class="table table-striped">
                    <thead>
   <th>N°patient</th><th>Nom</th><th>Prenom</th><th>telephone</th><th>adresse</th><th>email</th><th>Date de naissance</th><th>sit_fam</th><th>sexe</th><th>Time</th><th>Action</th>
                   </thead>
                        <tbody>
                        <?php
                        ...
                        ?>
                             <tr>
                                ...
                                <td><?php echo $user['jour']?>/<?php echo $user['mois'] ?>/<?php echo $user['annee']?></td>
                                <td><?php echo $user['sit_fam']?></td>
                                <td><?php echo $user['sexe']?></td>
                                <td><?php echo $user['time']?></td>
                                <td>
 <a href="fichePat.php?id=<?php echo $user['id']?>">
  <button class="btn btn-primary"data-toggle="modal" data-target="#exampleModalLong"style="color: #fff;background-color: #4e97ff;border-color: #4e97ff;margin-top: -5px;"type="submit"> CONSULTER </button>
</a>
...
<?php } ?>                 
    ...
    <li class="page-item <?= ($currentPage == 1) ? "disabled" : "" ?>">
            <a href="ListePat?page=<?= $currentPage - 1 ?>" class="page-link">Précédente</a>
        </li>
        <?php for($i= 1; $i <= $pages; $i++): ?>
            <!-- Lien vers chacune des pages (activé si on se trouve sur la page correspondante) -->
            <li class="page-item <?php if($i == $currentPage)  echo "active";  ?>">
                <a href="listePat.php?page=<?= $i ?>" class="page-link"><?= $i ?></a>
            </li>
        <?php endfor ?>
           <!-- Lien vers la page suivante (désactivé si on se trouve sur la dernière page) -->
           <li class="page-item <?= ($currentPage == $pages) ? "disabled" : "" ?>">
            <a href="ListePat?page=<?= $currentPage +1 ?>" class="page-link">Suivante</a>
        </li>
