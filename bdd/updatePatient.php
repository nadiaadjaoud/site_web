...
$sth = $dbco-> prepare ('UPDATE  rdv set nom=:nom, prenom=:prenom, telephone=:telephone,jour=:jour,mois=:mois,annee=:annee WHERE id=:id LIMIT 1 ');

$sth->bindValue(':id',$_POST['id']);
$sth->bindValue(':nom',$_POST['nom']);
$sth->bindValue(':prenom',$_POST['prenom']);
$sth->bindValue(':telephone',$_POST['telephone']);
$sth->bindValue(':jour',$_POST['jour']);
$sth->bindValue(':mois',$_POST['mois']);
$sth->bindValue(':annee',$_POST['annee']);

//execution de la requete
 $sth -> execute();

 $erreur = "votre patient a été bien modifier veuillez cliquer ! <a href=\"../html/listeRdvm.php\">ici</a>"
